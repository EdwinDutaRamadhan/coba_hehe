        ...
        <nav id="sidebar">
            ...
            <ul class="list-unstyled components">
               ...
               <li class="dropdown"><!-- MANAJEMEN USER -->
                <a class="dropdown-toggle" {{ $parent->route_name == null ? 'data-bs-toggle=collapse' : '' }} href="{{ $parent->route_name !== null ? route($parent->route_name) : '#admin_' . $parent->feature_id }}" role="button"
                    aria-expanded="false" aria-controls="{{ 'admin_' }}">
                    <i class="{{ $parent->icon_name }} fs-4 px-auto" style="width: 28px;"></i>
                    <span>{{ $parent->name }}</span>
                </a>
                <ul class="collapse {{ Request::segment(2) == strtolower(str_replace(' ', '-', $parent->name)) ? 'show' : '' }}" id="{{ 'admin_'.$parent->feature_id }}">
                    @foreach ($role->child as $child)
                        @if ($child->parent_id == $parent->feature_id)
                            <li class="{{ Request::segment(3) == 'admin' ? 'active' : '' }}">
                                <a href="{{ route($child->route_name) }}"><i class="{{ $child->icon_name }} fs-4 ps-4" style="width: 48px;"></i>
               ...
               {{-- @foreach (auth()->user()->role->parent as $parent)
               <li class="dropdown"><!-- MANAJEMEN USER -->
                ...
                <ul class="collapse show" id="admin_panel">
                    @foreach (auth()->user()->role->child as $child)
                        @if ($child->parent_id == $parent->feature_id)
                            <li class="{{ Request::segment(3) == 'admin' ? 'active' : '' }}">
